	...
	<div class="content">
		<div class="main role-detail">
			<div class="form-title">
				<div class="split-line"></div><div class="title">角色配置</div>
			</div>
			<div class="role-container">
				<el-form :inline="formLine" :label-position="labelPosition" label-width="70px" :model="roleAddDto"   :rules="rules" ref="ruleForm">
				    <el-form-item label="角色名称">
				    	<el-input v-model="roleAddDto.name" :disabled="true"></el-input>
				    </el-form-item>
				    <el-form-item label="描述">
				    	<el-input v-model="roleAddDto.description"></el-input>
				    </el-form-item>
				    <el-form-item label="">
...
export default {
	...
	methods: {
			...
			if (self.removeSelection.length === 0 ) {
				...
				self.$confirm('确定移除用户吗?', '提示', {
	              ...
	              type: 'warning'
	                ...
	                self.$api.post(self.url.removeRole, {roleID: self.getId(), users: self.removeSelection},function(r) {
	                    if (r.code==="ACK") {
		...
		setRoleId () {
			var self = this;
        	var roleId = self.getId();
			self.condition.roleId = roleId;
            self.searchDto.roleId = roleId;
		},
		getId(){
            var url = location.href; 
            var s = location.href.indexOf('?'); 
            return url.substr(s).split('=')[1];
        },
        initRole () {
        	var self = this;
        	var roleId = self.getId();
        	self.$api.get(self.url.getRole + roleId, {}, function(r) {
                if (r.code==="ACK"){
                    self.roleAddDto = r.data;
                }else{
                    self.$message.error(r.message);
                }
		...
		handleRemoveSelectionChange (val) {
			...
			for (var item of val) {
				self.removeSelection.push(item.userId);
			}
		...
		addUserRole () {
			...
			if (self.multipleSelection.length === 0) {
				...
				self.$api.post(self.url.assignRole, {roleID: self.getId(), users: self.multipleSelection}, function(r) {
	                if (r.code === 'ACK') {
		...
		initTable2 () {
			var self = this;
			self.$api.post(self.url.noRoleUsers, self.searchDto, function(r) {
                console.log(r);
                if (r.code === 'ACK') {
		...
		initTable () {
	    	var self = this;
			self.$api.post(self.url.haveRoleUsers, self.condition, function(r) {
                console.log(r);
                if (r.code === 'ACK') {
